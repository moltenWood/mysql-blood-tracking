...
var sqlTarget1 = `SELECT (SELECT g g1, h h1 FROM i LEFT JOIN j), a FROM (SELECT (d) d1) b FROM WHERE (SELECT (f) f1, ()), c FROM (f) f1`

func Test_main(t *testing.T) {
	controllers.ShowIndex(sqlTarget1)
	var ret0 = strings.Replace(utils.DeleteExtraSpace(sqlTarget1), "\n", "", -1)
	var collector = new(controllers.Collector)
	controllers.GetArrayTree(ret0, &controllers.InnerBracketId, collector)
	controllers.ShowCollector(collector)
}
